...
public class TestCommandClass implements CommandCase {
	...
	public void test6(CommandContext context){
		...
		Material[] materials = new Material[]{
				Material.STONE, Material.GRASS, Material.DIRT, Material.WOOL, Material.BEACON, Material.BEDROCK,
				Material.BED,
		...
		PageableInventory inventory =
				new PageableInventory("Title", InventorySize.FOUR_ROWS, items, new SimpleItem(Material.STAINED_GLASS),
	...
	public void test8(CommandContext context){
		...
		if(TestPlugin.testMob == null){
			TestPlugin.testMob =
					new FakeMob(MobType.VILLAGER, loc, "&cTest Villager", new EntitySettings(false, true, false));
		}
		TestPlugin.testMob.spawn(player);

		messager.write("&eMob spawned. &7[&b" + NPCRegistry.getEntities().size() + "&7]", true, MessageChannel.CHAT,
				player);
	}
	...
	public void test10(CommandContext context){
		...
		NametagManager.setNametag(false, "&cAdmin &8| &r", "", Collections.singletonList(player),
				BukkitUtilities.onlinePlayers());
	}
	...
	public void test11(CommandContext context){
		Player player = (Player) context.getSender();
		PlayerMessager messager = new PlayerMessager("&9Test &8| ");

		ParticleEffect effect = ParticleEffect.CLOUD;
		ParticleInformation data = new ParticleInformation(LocationUtil.fix(player.getLocation()), 1, effect);

		BukkitUtilities.showParticle(data, player);
