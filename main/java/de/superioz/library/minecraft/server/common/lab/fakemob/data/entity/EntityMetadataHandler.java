...
import de.superioz.library.minecraft.server.common.lab.fakemob.root.holo.CraftHologramPart;
...
public class EntityMetadataHandler {
    ...
    public static WrappedDataWatcher getHologramDefaults(CraftHologramPart part){
        ...
        dataWatcher.setObject(EntityMetadata.ARMORSTAND_STATE.getIndex(), (byte) EntityMetadataValues
                .ArmorStandStateMeta.SMALL.v());
        dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_HEALTH.getIndex(), 1.0F);
        dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_POTION_EFFECT_COLOR.getIndex(), 0);
        dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_POTION_EFFECT_AMBIENT.getIndex(), (byte) 0);
        dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_ARROWS_INSIDE.getIndex(), (byte) 0);
        dataWatcher.setObject(EntityMetadata.ENTITY_STATE.getIndex()
                , ProtocolUtil.toByte(EntityMetadataValues.EntityStateMeta.INVISIBLE.v()));
        dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_SHOWNAME.getIndex(), ProtocolUtil.toByte(true));

        if(part.getText() != null && !part.getText().isEmpty()){
            dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_NAME.getIndex(),
                    ChatUtil.colored(part.getText()));
            dataWatcher.setObject(EntityMetadata.LIVING_ENTITY_SHOWNAME.getIndex(),
    ...
    public static WrappedDataWatcher setMetadataValue(WrappedDataWatcher dataWatcher
            , EntityMetadata metadata, Object value){
